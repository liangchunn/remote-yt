...
function getHistoryPageItems(
  ...
  const pageItems: HistoryPageItem[] = [];
  let previousPage = 0;
...
function HistoryContainer({
  ...
  const pageCount = Math.ceil((data?.length ?? 0) / HISTORY_PAGE_SIZE);
  const currentPage = Math.min(page, Math.max(pageCount, 1));
  const startIndex = (currentPage - 1) * HISTORY_PAGE_SIZE;
  const pageEntries = data?.slice(startIndex, startIndex + HISTORY_PAGE_SIZE);
  const pageItems = getHistoryPageItems(currentPage, pageCount);

  const goToPage = (event: MouseEvent<HTMLAnchorElement>, page: number) => {
    event.preventDefault();
    setPage(Math.min(Math.max(page, 1), Math.max(pageCount, 1)));
  };
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-max min-w-max">
                  ...
                  <DropdownMenuItem
                    ...
                    className="whitespace-nowrap"
                    onClick={() =>
                      removeHistoryMutation.mutate(entry.webpage_url)
                    }
                  >
                ...
                <PaginationItem key={pageItem}>
                  {typeof pageItem === "number" ? (
                    <PaginationLink
                      ...
                      onClick={(event) => goToPage(event, pageItem)}
                    >
                      {pageItem}
                    </PaginationLink>
                  ) : (
